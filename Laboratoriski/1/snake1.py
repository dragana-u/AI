...
if __name__ == '__main__':
    num_of_green_apples = int(input())
    green_appls = []
    for i in range(num_of_green_apples):
        g_input = input().split(",")
        green_appls.append((int(g_input[0]), int(g_input[1])))

    num_of_red_apples = int(input())
    red_appls = []
    for j in range(num_of_red_apples):
        r_input = input().split(",")
