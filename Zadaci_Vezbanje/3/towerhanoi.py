    ...
    def successor(self, state):
        ...
        for i in range(len(tmp)):
            for j in range(len(tmp)):
                if len(tmp[i]) != 0 and i != j:
                    to_move = tmp[i][-1]
                    if len(tmp[j]) == 0 or to_move <= tmp[j][-1]:
                        new_state = [list(subtuple) for subtuple in tmp]
                        new_state[i].remove(to_move)
                        new_state[j].append(to_move)
                        new_state = [tuple(subtuple) for subtuple in new_state]
...
if __name__ == "__main__":
    start = tuple(map(tuple, input().replace(',', '').split(';')))
    end = tuple(map(tuple, input().replace(',', '').split(';')))
    hanoi = Hanoi(start, end)
