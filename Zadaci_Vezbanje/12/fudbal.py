...
class SoccerMatch(Problem):
    ...
    def successor(self, state):
        ...
        new_state = ((player[0], player[1] + 1), ball)
        if self.check_valid(new_state) and player[1] != new_state[0][1]:
        ...
        new_state = ((player[0] + 1, player[1]), ball)
        if self.check_valid(new_state) and player[0] != new_state[0][0]:
            successors["Pomesti coveche desno"] = new_state
        # Pomesti coveche gore-desno
        new_state = ((player[0] + 1, player[1] + 1), ball)
        if self.check_valid(new_state) and new_state[0] != player:
        ...
        # Turni topka gore
        new_state = ((player[0], player[1] + 1), (ball[0], ball[1] + 1))
        if self.check_valid(new_state) and ball[1] != new_state[1][1] and player[1] != new_state[0][1] and ball == \
                new_state[0]:
            successors["Turni topka gore"] = new_state
        # Turni topka dolu
        new_state = ((player[0], player[0] - 1), (ball[0], ball[1] - 1))
        if self.check_valid(new_state) and ball[1] != new_state[1][1] and player[1] != new_state[0][1] and ball == \
                new_state[0]:
            successors["Turni topka dolu"] = new_state
        # Turni topka desno
        new_state = ((player[0] + 1, player[1]), (ball[0] + 1, ball[1]))
        if self.check_valid(new_state) and ball[0] != new_state[1][0] and player[0] != new_state[0][0] and ball == \
                new_state[0]:
            successors["Turni topka desno"] = new_state
        # Turni topka gore-desno
        new_state = ((player[0] + 1, player[1] + 1), (ball[0] + 1, ball[1] + 1))
        if self.check_valid(new_state) and state != new_state and ball == new_state[0]:
    ...
    def check_valid(self, state):
        player = state[0]
        ball = state[1]
        # throw = ((ball[0] - 1, ball[1]), (ball[0] + 1, ball[1]), (ball[0], ball[1] - 1), (ball[0], ball[1] + 1))
        opponent_catch = ((2, 2), (3, 2), (4, 2), (2, 3), (4, 3), (5, 3), (6, 3),
                          (2, 4), (3, 4), (4, 4), (6, 4),
                          (4, 5), (5, 5), (6, 5))
        if player == ball:
            return False
        if player in self.obstacles:
            return False
        if player[0] >= self.width or player[1] >= self.height:
            return False
        if player[0] < 0 or player[1] < 0:
            return False
        if ball in self.obstacles:
            return False
        if ball[0] >= self.width or ball[1] >= self.height:
            return False
        if ball[0] < 0 or ball[1] < 0:
            return False
        if ball in opponent_catch:
            return False
        # if player not in throw:
        #     return False
        return True
