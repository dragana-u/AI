...
class GhostOnSkates(Problem):
    ...
    def successor(self, state):
        ...
        for i in range(1, 4):
            new_state = move_up(ghost, i)
            if self.check_valid(new_state, self.walls, self.n):
                successors[f'Gore {i}'] = new_state
        for i in range(1, 4):
            new_state = move_right(ghost, i)
            if self.check_valid(new_state, self.walls, self.n):
                successors[f'Desno {i}'] = new_state
        return successors

    def h(self, node):
        g_x, g_y = node.state[0], node.state[1]
        p_x, p_y = goal_pos[0], goal_pos[1]
        return (abs(g_x - p_x) + abs(g_y - p_y)) / 3
