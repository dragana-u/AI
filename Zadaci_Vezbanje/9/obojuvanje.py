    ...
    def successor(self, state):
        ...
        for i in range(self.size):
            for j in range(self.size):
                tmp = [list(t) for t in state]
                tmp[i][j] = invert(tmp[i][j])
                if i - 1 >= 0:
                    tmp[i-1][j] = invert(tmp[i-1][j])
                if i + 1 < self.size:
                    tmp[i+1][j] = invert(tmp[i+1][j])
                if j - 1 >= 0:
                    tmp[i][j-1] = invert(tmp[i][j-1])
                if j + 1 < self.size:
                    tmp[i][j+1] = invert(tmp[i][j+1])
                to_tuple = [tuple(t) for t in tmp]
                successors[f'x: {i}, y: {j}'] = tuple(to_tuple)
        return successors
...
if __name__ == "__main__":
    n = int(input())
    boi = tuple(map(int, input().split(",")))
    rows = []
    for i in range(n):
        rows.append(boi[i * n: (i + 1) * n])
    rows = tuple(rows)
    problem = Boi(rows, n)
