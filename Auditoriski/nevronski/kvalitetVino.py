...
def divide_sets(dataset):
    ...
    test_set = bad_classes[int(len(bad_classes) * 0.8):] + good_classes[int(len(good_classes) * 0.7):]
...
if __name__ == '__main__':
    ...
    print(f'Klasfk {i} ima tocnost so validacisko m-vo od {val_acc}')
    if val_acc > max_acc:
        max_acc = val_acc
        final_classifier = c
    ...
    for true, pred in zip(test_y, predictions):
        if true == pred:
            acc += 1
    acc = acc / len(test_y)
    print(f'Tocnosta so testiracko m-vo e {acc}')