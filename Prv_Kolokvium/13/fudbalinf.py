...
class Football(Problem):
    def __init__(self, initial, obstacles, goals):
    ...
    def check_valid(state, opponents):
        ...
        return man_pos[0] >= 0 and man_pos[0] < 8 and \
            ...
            man_pos != ball_pos and \
            (ball_pos[0] + 1, ball_pos[1]) not in opponents and \
            (ball_pos[0] - 1, ball_pos[1]) not in opponents and \
            (ball_pos[0], ball_pos[1] + 1) not in opponents and \
            (ball_pos[0], ball_pos[1] - 1) not in opponents and \
            (ball_pos[0] + 1, ball_pos[1] - 1) not in opponents and \
            (ball_pos[0] - 1, ball_pos[1] + 1) not in opponents

    @staticmethod
    def push_ball(man, ball):
        return man == tuple(ball)
    ...
    def successor(self, state):
        ...
        new_ball = tuple(ball)
        if self.push_ball(new_man, ball):
            new_ball = (ball[0], ball[1] + 1)
        new_state = (new_man, new_ball)
        if self.check_valid(new_state, self.obstacles):
            if new_ball != tuple(ball):
    ...
    def h(self, node):
        man = node.state[0]
        ball = node.state[1]
        r1 = abs(man[0] - ball[0]) + abs(man[1] - ball[1])
        r2 = abs(ball[0] - self.goals[0][0]) + abs(ball[1] - self.goals[0][1])
        r3 = abs(ball[0] - self.goals[1][0]) + abs(ball[1] - self.goals[1][1])
        r4 = min(r2, r3)
        return min(r1, r4)/5
...
if __name__ == '__main__':
    ...
    opponents = ((3, 3), (5, 4))
    goals = ((7, 2), (7, 3))
    fudbal = Football((man_pos, ball_pos), opponents, goals)
    result = astar_search(fudbal)
    if result is not None:
