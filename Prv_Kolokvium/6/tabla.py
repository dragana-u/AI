...
class Squares(Problem):
    ...
    def successor(self, state):
        ...
        for x in range(1, 6):
            if self.in_pos(sq[x - 1], x - 1):
                continue
            new_state = list(sq)
            new_state[x - 1] = (pomesti_gore(sq[x - 1]))
            if self.check_valid(new_state):
                succ[f'Pomesti kvadratche {x} gore'] = tuple(new_state)

            new_state = list(sq)
            new_state[x - 1] = (pomesti_dolu(sq[x - 1]))
            if self.check_valid(new_state):
                succ[f'Pomesti kvadratche {x} dolu'] = tuple(new_state)

            new_state = list(sq)
            new_state[x - 1] = (pomesti_desno(sq[x - 1]))
            if self.check_valid(new_state):
                succ[f'Pomesti kvadratche {x} desno'] = tuple(new_state)

            new_state = list(sq)
            new_state[x - 1] = (pomesti_levo(sq[x - 1]))
            if self.check_valid(new_state):
                succ[f'Pomesti kvadratche {x} levo'] = tuple(new_state)
        return succ
