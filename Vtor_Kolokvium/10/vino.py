...
if __name__ == '__main__':
    ...
    test_x_removed = []
    for t in test_x:
        r = [t[i] for i in range(len(t)) if i != least_important]
        test_x_removed.append(r)
    train_x_removed = []
    for t in train_x:
        r = [t[i] for i in range(len(t)) if i != least_important]
        train_x_removed.append(r)

    standard_scaler = StandardScaler()
    standard_scaler.fit(train_x_removed)

    min_max_scaler = MinMaxScaler()
    min_max_scaler.fit(train_x_removed)

    classifier = MLPClassifier(15, activation='relu', learning_rate_init=0.001, max_iter=200, random_state=0)


    classifier.fit(standard_scaler.transform(train_x_removed), train_y)
    print(f'Tocnost so StandardScaler: {classifier.score(standard_scaler.transform(test_x_removed), test_y)}')

    classifier.fit(min_max_scaler.transform(train_x_removed), train_y)
    print(f'Tocnost so MinMaxScaler: {classifier.score(min_max_scaler.transform(test_x_removed), test_y)}')