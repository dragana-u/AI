...
def main(dataset):
    ...
    # train_set = dataset[:int(len(dataset) * train_size)]
    # test_set = dataset[int(len(dataset) * train_size):]
    #
    ...
    for label in [0, 1]:
        ...
        train_x.extend(X_train_)
        test_x.extend(X_test_)
        train_y.extend(y_train_)
        test_y.extend(y_test_)
    ...
    best_classifier_index = accuracies.index(max(accuracies))
    index_to_name = {
        0: "Naive Bayes",
        1: "Decision Tree",
        2: "Random Forest",
    ...
    print(f'Najgolema tocnost ima klasifikatorot {index_to_name[best_classifier_index]}')

    final_predictions = []
    for x in test_x:
        predictions = [model.predict([x])[0] for model in models]
        prediction = sum(predictions) + predictions[best_classifier_index]
        final_predictions.append(1 if prediction > 2 else 0)

    print(f'Odzivot na kolekcijata so klasifikatori e {recall_score(test_y, final_predictions)}')
