...
if __name__ == '__main__':
    ...
    index_to_be_removed = int(input())
    to_be_classified = [list(map(float, input().split(" ")))]
    to_be_classified_removed = [row[:index_to_be_removed] + row[index_to_be_removed + 1:] for row in to_be_classified]

    dataset_removed = []
    for r in dataset:
        row = [r[i] for i in range(len(r)) if i != index_to_be_removed]
        dataset_removed.append(row)

    train_set = dataset[:int(0.8 * len(dataset))]
    val_set = dataset[int(0.8 * len(dataset)):]
    train_set_removed = dataset_removed[:int(0.8 * len(dataset_removed))]

    train_x = [row[:-1] for row in train_set]
    val_x = [row[:-1] for row in val_set]
    train_x_removed = [row[:-1] for row in train_set_removed]

    train_y = [row[-1] for row in train_set]
    val_y = [row[-1] for row in val_set]
    train_y_removed = [row[-1] for row in train_set_removed]
    ...
    classifier = MLPClassifier(num_neurons, activation="relu", max_iter=20, learning_rate_init=learning_rate, random_state=0)
    classifier.fit(train_x, train_y)

    acc1, acc2 = 0, 0

    for p, true in zip(classifier.predict(train_x), train_y):
        if p == true:
            acc1 += 1
    for p, true in zip(classifier.predict(val_x), val_y):
        if p == true:
            acc2 += 1

    acc1 = acc1 / len(train_set)
    acc2 = acc2 / len(val_set)

    if acc1 > acc2 * 1.15:
        print("Se sluchuva overfitting")
        scaler.fit(train_x_removed)
        classifier.fit(scaler.transform(train_x_removed), train_y_removed)
        print(classifier.predict(scaler.transform(to_be_classified_removed))[0])
    else:
        print("Ne se sluchuva overfitting")
        print(classifier.predict(scaler.transform(to_be_classified))[0])
